...
import numpy as np
import os
threshold = 20 #valor de pixel a partir del cual delimita el area mayor a estudiar
#====================== adaptiveThreshold ============================
blockSize = 33 # en el mpetodo adaptative threshold es el tamaño en pixeles del cuadro de exploración
C = 5 # Factor multiplicativo flotante que puede admitir mas o menos sensibilidad de deteccion

path = "F:/MVI_0310.mp4"
name = 'MVI_0310_13_agost(prueba_50_20)_total.xlsx'
# Escribimos un archivo de texto para guardar los parametros con lo que se llevo  cabo la prueba
file = open("reporte"+name+".txt", "w")
...
for k in range(int(video.get(cv2.CAP_PROP_FRAME_COUNT))):
    ...
    ret, grayMain = cv2.threshold(gray,threshold,255,cv2.THRESH_BINARY)
    ...
    cv2.imshow("Imagen procesada Areas de Deteccion", frame)
    ...
    if numDatos > 20:
        ...
        radio = 1;
    print('area',areaTotalnegativa,'radio ',radio,'area_mayor',arrayMain[indiceMain])
    hoja.insert_rows(k+2)
    hoja.append((areaTotal,x,y,radio,areaTotalnegativa,arrayMain[indiceMain],arrayMain[indiceMain]-areaTotalnegativa))
    print('guardando datos en fila ',k)
    #wb.save('MVI_0310_12_agost(prueba_50_20-12).xlsx')
    cv2.waitKey(1)
wb.save(name)
file.write("adaptiveThreshold" + os.linesep)
file.write("blockSize = "+ str(blockSize) +os.linesep)
file.write("C (factor)= "+ str(C) + os.linesep)
file.write("C (factor)= 5"+ os.linesep)
file.write("threshold ="+str(threshold) + os.linesep)
